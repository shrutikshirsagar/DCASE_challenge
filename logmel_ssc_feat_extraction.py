...
def mfcc(signal,samplerate=16000,winlen=0.040,winstep=0.020,numcep=39,
         nfilt=40,nfft=None,lowfreq=0,highfreq=None,preemph=0.97,ceplifter=22,appendEnergy=True,
...
def fbank(signal,samplerate=16000,winlen=0.040,winstep=0.020,
...
def logfbank(signal,samplerate=16000,winlen=0.040,winstep=0.020,
             nfilt=40,nfft=512,lowfreq=0,highfreq=None,preemph=0.97,
             winfunc=lambda x:numpy.ones((x,))):
...
def ssc(signal,samplerate=16000,winlen=0.040,winstep=0.020,
        nfilt=40,nfft=512,lowfreq=0,highfreq=None,preemph=0.97,
        winfunc=lambda x:numpy.ones((x,))):
...
def listfolders():
    ...
    pathin = '/home/shruti/Documents/dev_data/test'
    pathout = '/media/shruti/Data/Modulation_folder/Task_2/test'
    ftype = 3

    for dirname in os.listdir(pathin):
        print("Processing: %s/%s" % (pathin, dirname))
        if os.path.isdir("%s/%s" % (pathin, dirname)):
            mrs_dir = "%s/%s/%s" % (pathout, dirname, "logfbank")
            msf_dir = "%s/%s/%s" % (pathout, dirname, "ssc")
            
            print("ftype has been defined as : %s", ftype)
            pool_msr(pathin, dirname, mrs_dir, msf_dir,ftype = 3)
